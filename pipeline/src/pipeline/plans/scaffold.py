...
def scaffold_all(
    ...
    for lewis_path in targeted:
        result: ScaffoldResult = scaffold_one(
            lewis_path, plans_dir, state, lewis_commit_sha, raw_dir, force
        )
...
def scaffold_one(
    ...
    source_file: str = str(lewis_path.relative_to(raw_dir))
    ...
    try:
        content: str = _render_plan_yaml(plan)
...
def _render_plan_yaml(plan: Plan) -> str:
    """Serialize a Plan as YAML with a header comment block."""
    plan_dict: dict[str, Any] = plan.model_dump(mode="json")
    header: str = (
        f"# Generated by `pipeline scaffold-plans` from {plan.source_file}.\n"
