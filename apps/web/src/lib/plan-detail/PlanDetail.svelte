<script lang="ts">
	...
	import { buildCitationNumbers, citationAnchorId } from './citations';
	...
	const citationNumbers = $derived(plan ? buildCitationNumbers(plan) : new Map<string, number>());
	...
	<section>
		...
		{#if isRealProse(plan.origin_details)}
			<div class="mt-6">
				...
				<Prose text={plan.origin_details} numbers={citationNumbers} planId={plan.plan_id} />
			</div>
		{/if}
		...
		{#if isRealProse(plan.struck_down_details) || plan.struck_down_districts.length > 0}
			<div class="mt-6">
				...
				{#if isRealProse(plan.struck_down_details)}
					<Prose text={plan.struck_down_details} numbers={citationNumbers} planId={plan.plan_id} />
				{/if}
			...
			<div class="mt-6">
				...
				<ul class="mt-2 flex flex-col gap-2">
					{#each plan.court_citations as cite (cite.citation)}
						{@const citeNum = cite.source_code ? citationNumbers.get(cite.source_code) : undefined}
						<li
							id={cite.source_code && citeNum !== undefined
								? citationAnchorId(plan.plan_id, cite.source_code)
								: undefined}
							class="text-ink-secondary target:bg-surface-sunken scroll-mt-4 rounded text-sm"
						>
							{#if citeNum !== undefined}<span class="text-ink-muted mr-1">[{citeNum}]</span>{/if}
							<a
								href={cite.url}
								target="_blank"
